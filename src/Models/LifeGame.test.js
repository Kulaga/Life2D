...
test("random cell should dead because of overpopulation", () => {
    const sut = new LifeGame(4, 4);
    const currentState = [
        [CellState.Dead, CellState.Dead, CellState.Dead, CellState.Dead],
    ...
    const resultingNumberOfAlive = _.flatten(result).filter(c => c == CellState.Alive).length;
    expect(resultingNumberOfAlive).toStrictEqual(3);
})
